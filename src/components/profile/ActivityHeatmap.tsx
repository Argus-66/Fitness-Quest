...
export default function ActivityHeatmap({ workouts }: ActivityHeatmapProps) {
  ...
  const getColorClass = (level: number) => {
    switch (level) {
      case 0: return 'bg-theme-primary/20 border border-theme-primary/30'; // Darker empty cells
      case 1: return 'bg-theme-primary/30 border border-theme-primary/40 shadow-glow-sm';
      case 2: return 'bg-theme-primary/50 border border-theme-primary/60 shadow-glow-sm';
      case 3: return 'bg-theme-accent/70 border border-theme-accent/80 shadow-glow';
      case 4: return 'bg-gradient-to-br from-theme-accent to-theme-primary border border-theme-accent/80 shadow-glow-lg';
      default: return 'bg-theme-primary/20 border border-theme-primary/30'; // Darker empty cells
    }
  };
  ...
  return (
    <div className="bg-theme-dark/40 backdrop-blur-lg rounded-lg border border-theme-primary/30 p-4">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center space-x-3">
          <button 
            ...
            className="p-1.5 rounded-full bg-theme-primary/20 hover:bg-theme-primary/40 text-theme-light transition-colors duration-200 shadow-glow-sm"
            aria-label="Previous Year"
          >
            <FaChevronLeft size={14} />
          </button>
          
          <span className="text-theme-accent font-medium px-3 py-1.5 bg-theme-primary/10 rounded-md border border-theme-primary/30 shadow-glow-sm">
            {selectedYear}
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-br from-theme-dark/60 to-theme-primary/20 backdrop-blur-lg rounded-lg border border-theme-primary/30 p-4 flex items-center shadow-glow-sm">
          <div className="bg-gradient-to-br from-red-500 to-theme-accent rounded-full p-2.5 mr-3 shadow-glow">
            <FaFire className="text-white" size={18} />
          </div>
          <div>
            <div className="text-xs text-theme-light/80">Current Streak</div>
            <div className="text-xl font-bold text-white">{currentStreak} days</div>
          </div>
        </div>
        
        <div className="bg-gradient-to-br from-theme-dark/60 to-theme-primary/20 backdrop-blur-lg rounded-lg border border-theme-primary/30 p-4 flex items-center shadow-glow-sm">
          <div className="bg-gradient-to-br from-blue-500 to-theme-primary rounded-full p-2.5 mr-3 shadow-glow">
            <FaCalendarAlt className="text-white" size={18} />
          </div>
          <div>
            <div className="text-xs text-theme-light/80">Workouts in {selectedYear}</div>
            <div className="text-xl font-bold text-white">{totalWorkoutsThisYear}</div>
          </div>
        </div>
        
        <div className="bg-gradient-to-br from-theme-dark/60 to-theme-primary/20 backdrop-blur-lg rounded-lg border border-theme-primary/30 p-4 flex items-center shadow-glow-sm">
          <div className="bg-gradient-to-br from-green-500 to-theme-accent rounded-full p-2.5 mr-3 shadow-glow">
            <FaTrophy className="text-white" size={18} />
          </div>
          <div>
            <div className="text-xs text-theme-light/80">Best Streak</div>
            <div className="text-xl font-bold text-white">{Math.max(currentStreak, 7)} days</div>
          </div>
        </div>
      </div>
      
      <div className="overflow-x-auto bg-theme-dark/40 backdrop-blur-md rounded-lg border border-theme-primary/30 p-4 shadow-glow-sm">
        <div className="min-w-max">
          ...
          <div className="relative h-5 mb-2 ml-8">
            {monthPositions.map((month, i) => (
              <div 
                key={i} 
                className="absolute text-xs text-theme-light/90 font-medium"
          ...
          <div className="flex">
            ...
            <div className="w-8 flex flex-col mr-1">
              {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
                <div key={i} className="text-xs text-theme-light/80 h-4 flex items-center justify-center mb-1">
                  {day}
                </div>
              ))}
            </div>
                  ...
                  <div key={weekIndex} className="flex flex-col gap-1">
                    {week.map((day, dayIndex) => (
                      <div
                        ...
                        className={day ? `h-4 w-4 rounded-sm ${getColorClass(day.activityLevel)} cursor-pointer transition-all duration-200 hover:scale-110 hover:z-10 hover:border-theme-accent` : 'h-4 w-4'}
          ...
          {/* Legend */}
          <div className="flex justify-end items-center mt-6 text-xs text-theme-light/80">
            ...
            <span className="ml-2 text-theme-accent font-medium">More</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 text-center">
        <div className="inline-flex items-center space-x-2 bg-theme-dark/40 backdrop-blur-md px-4 py-2 rounded-lg border border-theme-primary/30 shadow-glow-sm">
          <FaDragon className="text-theme-accent" />
          <span className="text-theme-light/90 italic text-sm">
